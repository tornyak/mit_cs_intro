...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        ...
        for w in words:
            wLower = w.lower()
            foundIdx = wLower.find(wordLower)
            if foundIdx != -1:
                # check full word
                lowerOk = True
                upperOk = True
                if foundIdx > 0:
                    if wLower[foundIdx-1] not in string.punctuation: lowerOk = False

                if foundIdx + len(wordLower) < len(wLower):
                    if wLower[foundIdx + len(wordLower)] not in string.punctuation: upperOk = False

                if lowerOk and upperOk: return True
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType == "TITLE":
        ...
        otherTriger = triggerMap.get(params[0])
        if otherTriger:
            t = NotTrigger(otherTriger)
    elif triggerType == "AND":
        otherTriger1 = triggerMap.get(params[0])
        otherTriger2 = triggerMap.get(params[1])
        if otherTriger1 and otherTriger2:
            t = AndTrigger(otherTriger1, otherTriger2)
    elif triggerType == "OR":
        otherTriger1 = triggerMap.get(params[0])
        otherTriger2 = triggerMap.get(params[1])
        if otherTriger1 and otherTriger2:
            t = OrTrigger(otherTriger1, otherTriger2)
